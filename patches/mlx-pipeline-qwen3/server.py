...
class ModelProvider:
    ...
    def load(self, model_path, adapter_path=None, draft_model_path=None):
        ...
        if self.draft_model is None:
            ...
            # Disable batching for pipeline models (different eval sync needed)
...
class APIHandler(BaseHTTPRequestHandler):
    ...
    def handle_completion(self, request: CompletionRequest, stop_words: List[str]):
        ...
        for gen in response:
            ...
            with open('/tmp/mlx-token-debug.log', 'a') as _td:
                _td.write(f"in_r={in_reasoning} has_tc={ctx.has_tool_calling} in_tc={in_tool_call} tok={repr(gen.text)}\n")
            if in_reasoning:
                if gen.text == ctx.think_end:
                    in_reasoning = False
                else:
                    reasoning_text += gen.text
                    # Check for tool calls inside thinking output
                    if ctx.has_tool_calling and ctx.tool_call_start in reasoning_text:
                        # Tool call found inside thinking - extract it
                        before, _, after = reasoning_text.rpartition(ctx.tool_call_start)
                        reasoning_text = before
                        tool_text = after
                        made_tool_call = True
                        in_tool_call = True
                        in_reasoning = False
            elif ctx.has_tool_calling and not in_tool_call:
                # For multi-token tool_call_start (e.g. <|python_tag|>),
                # accumulate text and check if the delimiter has been completed
                text += gen.text
                segment += gen.text
                with open('/tmp/mlx-stream-debug.log', 'a') as _sd:
                    _sd.write(f"token={repr(gen.text)} text_tail={repr(text[-30:])} start={repr(ctx.tool_call_start)} match={ctx.tool_call_start in text}\n")
                if ctx.tool_call_start in text:
                    # Found tool call start in accumulated text
                    before, _, _ = text.partition(ctx.tool_call_start)
                    text = before
                    segment = ""
                    made_tool_call = True
                    in_tool_call = True
        ...
        # DEBUG
        with open('/tmp/mlx-tool-debug.log', 'a') as _dbg:
            _dbg.write(f"text={repr(text[:200])}\n")
            _dbg.write(f"reasoning={repr(reasoning_text[:200])}\n")
            _dbg.write(f"tool_calls={tool_calls}\n")
            _dbg.write(f"has_tool_calling={ctx.has_tool_calling}\n")
            _dbg.write(f"tool_call_start={repr(ctx.tool_call_start)}\n")
        # Post-process: detect tool calls in content for multi-token delimiters
        # (e.g. <|python_tag|> which is 6 tokens and missed by per-token detection)
        if not tool_calls and ctx.has_tool_calling and ctx.tool_call_start in text:
            before, _, tool_content = text.partition(ctx.tool_call_start)
            if ctx.tool_call_end:
                tool_content, _, _ = tool_content.partition(ctx.tool_call_end)
            tool_calls.append(tool_content.strip())
            text = before.strip()
