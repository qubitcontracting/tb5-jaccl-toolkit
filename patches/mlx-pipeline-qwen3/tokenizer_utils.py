...
class TokenizerWrapper:
    ...
    def __init__(
        ...
        # Skip check for multi-token delimiters (e.g. <|python_tag|> = 6 tokens)
        if tool_call_start and tool_call_start not in vocab:
            # Check if it can be encoded as multiple tokens (multi-token delimiter)
            encoded = tokenizer.encode(tool_call_start, add_special_tokens=False)
            if len(encoded) <= 1:
                # Single token not in vocab - disable
                self._tool_call_start = None
                self._tool_call_end = None
                self._tool_parser = None
        if tool_call_end and tool_call_end not in vocab:
            encoded = tokenizer.encode(tool_call_end, add_special_tokens=False)
            if len(encoded) <= 1:
                self._tool_call_start = None
                self._tool_call_end = None
                self._tool_parser = None
...
def load(
    ...
    _inferred = _infer_tool_parser(tokenizer.chat_template)
    tool_parser_type = tokenizer_config.get("tool_parser_type", _inferred)
    with open('/tmp/mlx-load-trace.log', 'a') as _tl:
        _tl.write(f"inferred={_inferred} final={tool_parser_type}\n")
    ...
    if tool_parser_type is not None:
        try:
            ...
            with open('/tmp/mlx-load-trace.log', 'a') as _tl:
                _tl.write(f"LOADED parser={tool_parser_type} start={repr(tool_call_start)}\n")
        except Exception as e:
            with open('/tmp/mlx-load-trace.log', 'a') as _tl:
                _tl.write(f"IMPORT FAILED: {e}\n")
            tool_parser = None
            tool_call_start = None
            tool_call_end = None
    else:
